...
export default function CoachProfile() {
  ...
  if (!coach) {
    return (
      <View className="flex-1 items-center justify-center bg-cream py-20">
        <Text className="font-heading text-xl font-bold text-navy">Coach not found 😕</Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView className="flex-1 bg-cream">
      {/* Hero band */}
      <View className="bg-navy px-4 pb-16 pt-10">
        <View
        ...
        >
          <View
            ...
            style={{
              ...
              ...(Platform.OS === "web"
                ? ({ backgroundImage: "linear-gradient(135deg,#FF6B35 0%,#1B2B4B 100%)" } as object)
                : {}),
            }}
          />
          <View className="items-center">
            {coach.profile_photo_url ? (
              ...
              <View className="h-28 w-28 items-center justify-center rounded-full border-4 border-white bg-saffron">
      ...
      <View className="mx-auto -mt-8 w-full max-w-4xl flex-col gap-4 px-4 pb-10 md:flex-row">
        ...
        <View className="flex-1 gap-4">
          ...
          {coach.bio ? (
            <Card>
              <Text className="mb-2 font-heading text-lg font-bold text-navy">About</Text>
                ...
                <View className="mt-4 flex-row flex-wrap gap-2">
                  {skills.map((s) => (
                    <View key={s} className="rounded-full bg-cream px-3 py-1.5">
                      <Text className="text-xs font-semibold text-navy">{s}</Text>
                    </View>
                  ))}
                </View>
              )}
          ...
          {coach.teaching_photos && coach.teaching_photos.length > 0 && (
            <Card>
              <Text className="mb-3 font-heading text-lg font-bold text-navy">Teaching Photos</Text>
        ...
        <View className="md:w-72">
          <Card>
            ...
            <Pressable
              onPress={onShare}
              className="mt-3 items-center rounded-xl border border-navy py-3 active:opacity-80"
            >
              <Text className="font-heading text-sm font-semibold text-navy">
